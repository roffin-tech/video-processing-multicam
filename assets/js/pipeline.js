...
export class Pipeline {
  ...
  constructor() {
    ...
    this.streamProcessor_ = new StreamProcessor();
    /**
  ...
  async maybeStartPipeline_() {
    ...
    const deviceList = await this.streamProcessor_.getMediaDevices();
    const streams = await this.streamProcessor_.getVideoStreams(deviceList);

    await this.frameTransform_.init();
    try {
      this.processedStream_ =
        await this.streamProcessor_.createProcessedMediaStream(
          streams[0],
